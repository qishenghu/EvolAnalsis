            ...
            for i, batch_dict in enumerate(self.train_dataloader):
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        logger.info(
                            f"📊 REWARD TENSOR: shape={_tlr.shape}, "
                            f"sum=[{_tlr_sums.min().item():.4f}, {_tlr_sums.mean().item():.4f}, {_tlr_sums.max().item():.4f}], "
                            f"nonzero=[{_nonzero_counts.min().item()}, {_nonzero_counts.float().mean().item():.0f}, {_nonzero_counts.max().item()}]"
                        )
