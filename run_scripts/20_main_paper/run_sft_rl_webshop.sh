#!/bin/bash
# ==============================================================================
# DUET Paper: WebShop SFT+RL Baseline
#
# Two-phase training:
...
run_sft_rl() {
    ...
    echo ""
    echo "============================================"
    echo " SFT+RL for webshop_${size}"
    echo " $(date '+%Y-%m-%d %H:%M:%S')"
    echo "============================================"

    # --- Phase 1: SFT (50 steps) ---
    echo "[Phase 1/2] SFT on teacher data (50 steps)..."
    cleanup_training "webshop_${size}_sft"
    run_experiment "$gpus" \
        "${CONFIG_DIR}/webshop_${size}_sft.yaml" \
        "webshop_${size}_sft"

    local sft_ckpt="checkpoints/agentevolver/webshop_${size}_sft/global_step_50/actor"
    if [ ! -d "$sft_ckpt" ]; then
        echo "ERROR: SFT checkpoint not found at $sft_ckpt"
        ls -d checkpoints/agentevolver/webshop_${size}_sft/global_step_*/actor 2>/dev/null || echo "No checkpoints found"
        return 1
    fi
    ...
    cleanup_training "webshop_${size}_sft_rl"
    run_experiment "$gpus" \
        "${CONFIG_DIR}/webshop_${size}_sft_rl.yaml" \
        "webshop_${size}_sft_rl"

    echo "[DONE] SFT+RL for webshop_${size} complete"
}

echo "============================================"
echo " DUET Paper: WebShop SFT+RL Baselines"
echo " $(date '+%Y-%m-%d %H:%M:%S')"
echo "============================================"
...
echo ""
echo "============================================"
echo " All WebShop SFT+RL baselines done."
echo " $(date '+%Y-%m-%d %H:%M:%S')"
echo "============================================"
