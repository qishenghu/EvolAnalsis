...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    if is_llm_reward.int().sum()!=batch.batch.size(0):
        metrics.update({
            ...
            "critic/score-env/max": torch.max(sequence_score[~is_llm_reward]).detach().item(),
            "critic/score-env/min": torch.min(sequence_score[~is_llm_reward]).detach().item(),
            # reward
        ...
        metrics.update({
            # score
            "critic/score-syn/mean": torch.mean(sequence_score[is_llm_reward]).detach().item(),
            "critic/score-syn/max": torch.max(sequence_score[is_llm_reward]).detach().item(),
            "critic/score-syn/min": torch.min(sequence_score[is_llm_reward]).detach().item(),
            # reward
