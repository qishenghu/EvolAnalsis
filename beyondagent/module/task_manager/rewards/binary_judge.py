...
class LlmAsJudgeBinaryRewardCalculator(RewardCalculator):
    ...
    """
    def __init__(self, model_name='qwq-plus', mean_score:float=0.5):
...
"""
        ...
        messages.append({"role":"user","content":USER_PROMPT.format(task=task_query,trajs=trajectory_text, running_mean=self._running_judge_mean, mean_score=self._mean_score)})
        with open("tmp.txt", "w") as f:
            f.write(USER_PROMPT.format(task=task_query,trajs=trajectory_text, running_mean=self._running_judge_mean, mean_score=self._mean_score))
        return messages
