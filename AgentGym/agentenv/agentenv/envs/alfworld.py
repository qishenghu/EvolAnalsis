...
class AlfWorldEnvClient(BaseEnvClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.test_data_start_index = test_data_start_index # Start index of test data
        assert self.test_data_start_index >= 0, "test_data_start_index must be >= 0"
        print(f"is_eval: {self.is_eval}, test_data_start_index: {self.test_data_start_index}")
    ...
    def reset(self, game: int, world_type: str = "Text") -> dict[str, Any]:
        # `game`` is the index number
        # if using evlaution for test data, then `game` should be in [test_data_start_index, test_data_start_index + game]
        # if self.is_eval:
        initial_game = game
        game = game + self.test_data_start_index
        print(f"game: {initial_game} Aligned Test Data Index-> {game}")
