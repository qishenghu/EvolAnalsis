...
class APIAgent:
    ...
    def generate(
        ...
        if random.random() < 0.3:
            print(f"[APIAgent] 使用随机温度: {temperature}")
        while True:
            try:
                ...
                print(f"[Rate Limit] 遇到速率限制错误，等待 60 秒后重试: {e}")
                time.sleep(60)
            except Exception as e:
                # 检查是否是 HTTP 429 错误（某些情况下可能不是 RateLimitError）
                error_str = str(e).lower()
                if "429" in error_str or "rate limit" in error_str or "rate_limit" in error_str:
                    print(f"[Rate Limit] 检测到速率限制错误，等待 60 秒后重试: {e}")
                    time.sleep(60)
                else:
                    print(f"[Error] API 调用出错，等待 1 秒后重试: {e}")
                    time.sleep(1)
