...
from .env_wrapper import aime_env_server
from .model import *
...
def create(create_query: CreateQuery):
    ...
    env = aime_env_server.create(create_query.id)
    return env

@app.post("/step", response_model=StepResponse)
def step(step_query: StepQuery):
    observation, reward, done, info = aime_env_server.step(
...
def observation(env_idx: int):
    return aime_env_server.observation(env_idx)

@app.post("/reset", response_model=str)
def reset(reset_query: ResetQuery):
    aime_env_server.reset(reset_query.env_idx, reset_query.id)
    return aime_env_server.observation(reset_query.env_idx)


@app.post("/close")
def close(body: CloseRequestBody):
    return aime_env_server.close(body.env_idx)
