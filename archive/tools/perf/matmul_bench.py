#!/usr/bin/env python
"""matmul_bench — sustained cuBLAS matmul throughput benchmark (fills memory, ~100% SM).

Usage:
    python matmul_bench.py 4 6              # occupy GPU 4 and GPU 6
    python matmul_bench.py 4,6              # same, comma-separated also works
    python matmul_bench.py 4 6 --mem 0.8    # only grab 80% of currently-free memory
    python matmul_bench.py 4 6 --size 4096  # smaller matmul (lower power draw)
...
Stop with Ctrl+C, or from another shell:  pkill -x matmul_bench
...
def ensure_process_name():
    ...
    try:
        ...
        print(f"[matmul_bench] warning: could not rename process ({e}); "
...
def occupy(gpu_id, mem_frac, size, stop_event):
    try:
        ...
        print(f"[matmul_bench] GPU {gpu_id} worker died: {e!r}", file=sys.stderr, flush=True)
...
def _occupy(gpu_id, mem_frac, size, stop_event):
    ...
    print(f"[matmul_bench] GPU {gpu_id}: holding {used:.1f} GiB, running matmul loop",
          flush=True)
...
def main():
    ...
    try:
        ...
        sys.exit(f"[matmul_bench] error: this python ({sys.executable}) has no torch — "
    ...
    print(f"[matmul_bench] pid {os.getpid()} occupying GPUs {gpus} "
    ...
    for t in threads:
        t.join(timeout=10)
    print("[matmul_bench] released all GPUs, bye")
