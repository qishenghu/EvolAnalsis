...
### 8.4 判别器输入特征（当前配置：`feature_mode=v3_aug`）

你现在的训练配置（`config/alfworld_grpo_3b_dr3_hidden.yaml`）里使用的是：
- `actor_rollout_ref.actor.dr3.feature_mode: "v3_aug"`

其判别器输入是**每条序列（sample）一个特征向量**，在代码里由 `agentevolver/module/exp_manager/dr3_ratio.py::compute_sequence_features()` 构造，计算口径为：
- 仅在 **response token** 上做聚合（由 `response_mask` 指定，来自 `loss_mask` 或 `attention_mask` 的 response slice）
- 全部特征在 `torch.no_grad()` 下计算（不回传梯度）

#### `v3_aug` 的 12 维特征清单（按代码堆叠顺序）

令每个 token 的 `log_prob` 为当前策略对该 token 的对数概率；mask 为 response token；response 长度为 `resp_len = sum(mask)`。

- **(1) `lp_mean`**：masked mean of `log_prob`
- **(2) `lp_std`**：masked std of `log_prob`
- **(3) `lp_min`**：masked min of `log_prob`
- **(4) `lp_max`**：masked max of `log_prob`
- **(5) `lp_low_ratio_10`**：response token 中 `log_prob < -10` 的比例
- **(6) `lp_low_ratio_20`**：response token 中 `log_prob < -20` 的比例
- **(7) `lp_high_ratio_1`**：response token 中 `log_prob > -1` 的比例
- **(8) `resp_len`**：response token 数（mask 计数，float）

此外，若存在 reference policy 的 `ref_log_prob`（当前实现只有在 `use_kl_loss=true` 且 batch 内带 `ref_log_prob` 时才会传入），还会构造 token 级差分：
$$
\Delta \log p := \log p_{\text{cur}} - \log p_{\text{ref}}
$$
并在 response token 上聚合出 4 个 “KL-to-ref 形状代理” 特征；若 `ref_log_prob` 不可用，则这 4 个特征**全部置 0**（维度仍保留）：

- **(9) `kl_ref_mean`**：masked mean of $\Delta \log p$
- **(10) `kl_ref_std`**：masked std of $\Delta \log p$
- **(11) `kl_ref_abs_mean`**：masked mean of $|\Delta \log p|$
- **(12) `kl_ref_pos_ratio`**：response token 中 $\Delta \log p > 0$ 的比例

> 直观解释：`v3_aug` 保持 “advantage-free（不泄漏 reward/adv）” 的叙事，同时通过更丰富的 logprob tail / head 比例与 KL-to-ref 的形状统计，提升判别器区分 on/off 分布的可辨识度，但避免引入高容量 hidden state（更易过拟合）。
